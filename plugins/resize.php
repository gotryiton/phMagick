...
class phMagick_resize{
     ...
     * @param $height
     */
    function resizeGTIO(phmagick $p , $width, $height){
    	//requires Crop plugin
    	//requires dimensions plugin
		...
		echo 'w: ' . $w . ' h: ' . $h . '<br>';
		echo 'width: ' . $width . ' hheight: ' . $height. '<br>';
		...
		$p->resize($width, $height)->crop($width, $height);
 ...
 /**
    ...
    function onTheFly(phmagick $p,$imageUrl, $width, $height, $exactDimentions = false, $webPath = '', $physicalPath=''){
        ...
        if (! file_exists($thumbnailFile)){
            $p->resize($width, $height, $exactDimentions);
        }
