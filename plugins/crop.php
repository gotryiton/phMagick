...
/**
     ...
     * @return unknown_type
     */
    function crop(phmagick $p, $width, $height, $top = 0, $left = 0, $gravity = phMagickGravity::None, $quality = '100', $compress_type = 'bzip'){
        $cmd  = $p->getBinary('convert');
        $cmd .= ' ' . $p->getSource() ;

        if (($gravity != '') || ($gravity != phMagickGravity::None) )  $cmd .= ' -gravity ' . $gravity ;

        $cmd .= " -compress $compress_type -quality $quality";
